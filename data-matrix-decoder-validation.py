...
"""Data Matrix live evaluation script.
...
- NO READ, TIMEOUT, and any non-matching payload are treated as WRONG.
...
class UiState:
    ...
    overlay: str = "INIT"
    # Optional bounding rect from the last successful decode (full-frame coords)
...
class DmtxHardTimeoutWorker:
    ...
    def __init__(self) -> None:
        # fork is best on Linux for cv2/numpy objects; matches your live script.
    ...
    def _child_main(conn) -> None:
        while True:
            ...
            try:
                ...
                try:
                    results = dmtx_decode(gray, max_count=int(max_symbols), timeout=int(timeout_ms))
                except TypeError:
...
class LiveDmtxEvaluator:
    ...
    def _open_csv(self) -> None:
        ...
        self._csv_writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "payload",  # decoded text or NO READ
                "timed_out",  # 0/1
                "is_match",  # 0/1
    ...
    def _set_overlay(self, text: str, rect: Optional[Rect] = None) -> None:
        with self._ui_lock:
    ...
    def _log_decode(
        ...
        self._csv_fh.flush()  # ensure persistence during workshop testing

    def _log_attempt_summary(self, attempt_i: int, ts: float, matches: int, result: str) -> None:
        assert self._csv_writer is not None
        self._csv_writer.writerow(
            {
                "type": "attempt",
    ...
    def _evaluation_worker(self) -> None:
        ...
        for attempt in range(1, self.attempts + 1):
            ...
            for di in range(1, self.decodes_per_attempt + 1):
                ...
                if self.log_images_remaining > 0:
                    ...
                    cv2.imwrite(str(self.log_dir / f"{ts_str}_roi_color_{idx:04d}.png"), roi)
                    cv2.imwrite(str(self.log_dir / f"{ts_str}_roi_gray_{idx:04d}.png"), gray)
                    self.log_images_remaining -= 1

                t0 = time.perf_counter()
                text, rect_roi, timed_out = self._dmtx.decode(gray, max_symbols=self.max_symbols, timeout_ms=self.decode_timeout_ms)
                ...
                rect_full = None
                if text and rect_roi is not None:
                ...
                self._log_decode(
                    ...
                    ts=ts,
                    decode_ms=decode_ms,
                    payload=("NO READ" if timed_out else payload),
                    timed_out=timed_out,
                    is_match=is_match,
                )
            ...
            ts2 = time.time()
            self._log_attempt_summary(attempt_i=attempt, ts=ts2, matches=matches, result=result)
            print(
        ...
        # End of evaluation: request shutdown
        self._stop_event.set()
    ...
    def _run_ui_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            org = (10, 30)
            cv2.putText(frame, overlay, org, cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 5, cv2.LINE_AA)
            cv2.putText(frame, overlay, org, cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2, cv2.LINE_AA)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Live Data Matrix evaluation runner")
